...
def lhs_powerlognormal(mean, stdev, nsample):
    lhd = lhs(1, samples = nsample) #Generates n samples for each variable
    c = mean
    s = stdev
    lhd = stats.powerlognorm.ppf(lhd, c, s)
    return lhd
...
def lhs_distribution(input_series, correlation_distributions, correlation_parameters, nsample):
    ...
    elif distribution == 'powerlognormal':
        lhd = lhs_powerlognormal(input_series.expected, input_series.standard_deviation, nsample)
